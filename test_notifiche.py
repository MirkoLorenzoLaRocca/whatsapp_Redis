...
import datetime
import os
from notifiche import ricevi_notifiche, invia_notifica
                    ...
                    while True:
                        ...
                        if redis_client.exists(f'chat:{username}:{contacts[contact_choice]}' ):
                            ...
                            for chat in chat_list:
                                ...
                                if chat[1]=='inviato>':
                                    redis_client.publish(f'chat:{username}:{contacts[contact_choice]}', '     '*8+f'>{chat[2]}\n'+'     '*8+f'  {formatted_date}')
                                    print(f'     '*8+f'>{chat[2]}\n'+'     '*8+f'  {formatted_date}')
                                else:
                                    redis_client.publish(f'chat:{username}:{contacts[contact_choice]}', f'<{chat[2]}\n  {formatted_date}')
                                    print(f'<{chat[2]}\n  {formatted_date}')
...
# controllo che il campo type del get.message sia diverso da none
'''def get_message():
    message = p.get_message()
    if message is not None and 'type' in message:
        if message['type'] != None:
            redis_client.publish(f'hai un messaggio da: {message["channel"].split(":")[2]}')
    else:
        return False'''
